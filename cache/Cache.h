	...
	virtual V operator[](const K & key)
	{
		...
		else //иначе - пытаемся прочитать его из файла
		{
			...
			if(massKV.size()>=limit)  // Если кэш полон - удаляем самый старый элемент и записываем полученный.
			{
				...
				massKV.erase(massTK.begin()->second); //Удаляем элемент по ключу время которого самое старое, т.е. он располагается первым в massTK
				massTK.erase(massTK.begin()); //Удаляем самый старый (самый первый) элемент из massTK
				
			}
			massTK[time(NULL)] = key; //добавляем в массТК новый ключ с текущем временем.
			massKV[key] = result; //Добавляем в массКВ значение по ключу.
			//*/
